...
export async function POST(req: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  ...
  try {
    ...
    const { error: updateError } = await supabaseAdmin
      .from('businesses')
      .update({ strip_image_url: publicUrl })
      .eq('slug', slug)

    if (updateError) {
      console.error('Update error:', updateError)
      return NextResponse.json({ error: 'Error guardando la URL de la imagen' }, { status: 500 })
    }

    return NextResponse.json({ strip_image_url: publicUrl })
  } catch (err) {
    console.error('Upload strip error:', err)
