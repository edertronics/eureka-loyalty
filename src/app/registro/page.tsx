  ...
  return (
    <div style={{ borderRadius: 16, overflow: 'hidden', background: primary, boxShadow: `0 12px 40px ${primary}55`, transition: 'all 0.4s ease' }}>
      <div style={{ padding: '14px 16px 10px', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
        <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 9, letterSpacing: '0.14em', textTransform: 'uppercase', fontWeight: 700, margin: 0, fontFamily: FONT_STACK }}>
          Easy Loyalty
        </p>
        <p style={{ color: '#fff', fontSize: 16, fontWeight: 900, margin: '3px 0 0', letterSpacing: '-0.02em', fontFamily: FONT_STACK }}>
          {name || 'Tu negocio'}
        </p>
      </div>
...
export default function RegistroPage() {
  ...
  // ── FORM ────────────────────────────────────────────────────────
  const STEPS = [{ n: 1, label: 'Negocio' }, { n: 2, label: 'Programa' }, { n: 3, label: 'Acceso' }]
    ...
    <main style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '32px 16px', background: `linear-gradient(160deg, ${TEAL} 0%, ${CARBON} 65%)`, fontFamily: FONT_STACK }}>
      ...
      <div style={{ width: '100%', maxWidth: 360 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          <img src="/img/logo-dark.png" alt="Easy Loyalty" style={{ height: 75, width: 'auto', filter: 'invert(1)', opacity: 0.95, display: 'inline-block' }} />
          {step === 1 && (
            <p style={{ color: `${GREEN}`, fontSize: 17, fontWeight: 500, margin: '14px 0 0', letterSpacing: '0.02em', fontFamily: FONT_STACK, opacity: 0.75 }}>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ borderRadius: 20, padding: 24, background: 'rgba(255,255,255,0.04)', border: `1px solid rgba(0,200,150,0.15)`, backdropFilter: 'blur(24px)', display: 'flex', flexDirection: 'column', gap: 18 }}>
            ...
            {step === 1 && (
              <>
                ...
                <button type="button"
                  ...
                  style={{ width: '100%', padding: '15px 0', borderRadius: 12, fontWeight: 900, fontSize: 15, cursor: (!form.name || !form.slug || slugStatus === 'taken' || slugStatus === 'checking') ? 'not-allowed' : 'pointer', opacity: (!form.name || !form.slug || slugStatus === 'taken' || slugStatus === 'checking') ? 0.3 : 1, border: 'none', fontFamily: FONT_STACK, letterSpacing: '-0.01em', background: GREEN, color: TEAL, transition: 'opacity 0.2s' }}>
                  Siguiente →
                </button>
              </>
            )}
            ...
            {step === 2 && (
              <>
                <div>
                  <h2 style={{ color: '#fff', fontSize: 18, fontWeight: 800, margin: '0 0 3px', letterSpacing: '-0.02em', fontFamily: FONT_STACK }}>Tu programa</h2>
                  <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: 12, margin: 0, fontFamily: FONT_STACK }}>Personaliza la experiencia de tus clientes</p>
                </div>
                ...
                <div>
                  <p style={{ color: `${GREEN}50`, fontSize: 9, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', margin: '0 0 8px', fontFamily: FONT_STACK }}>Así verán tu tarjeta</p>
                  <CardPreview name={form.name} primary={color} accent={accent} goal={parseInt(form.stamp_goal)} reward={form.reward_description} />
                </div>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button type="button" onClick={() => { if (form.reward_description) setStep(3) }} disabled={!form.reward_description}
                    style={{ flex: 2, padding: '13px 0', borderRadius: 12, fontWeight: 900, fontSize: 14, cursor: !form.reward_description ? 'not-allowed' : 'pointer', opacity: !form.reward_description ? 0.3 : 1, border: 'none', fontFamily: FONT_STACK, background: GREEN, color: TEAL }}>
                    Siguiente →
                  </button>
                </div>
              </>
            ...
            {step === 3 && (
              <>
                <div>
                  <h2 style={{ color: '#fff', fontSize: 18, fontWeight: 800, margin: '0 0 3px', letterSpacing: '-0.02em', fontFamily: FONT_STACK }}>Acceso a tu panel</h2>
                  <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: 12, margin: 0, fontFamily: FONT_STACK }}>Solo tú tendrás esta contraseña</p>
                </div>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button type="submit" disabled={loading || !form.admin_password || !form.admin_password_confirm}
                    style={{ flex: 2, padding: '13px 0', borderRadius: 12, fontWeight: 900, fontSize: 14, cursor: (loading || !form.admin_password || !form.admin_password_confirm) ? 'not-allowed' : 'pointer', opacity: (loading || !form.admin_password || !form.admin_password_confirm) ? 0.3 : 1, border: 'none', fontFamily: FONT_STACK, background: GREEN, color: TEAL }}>
